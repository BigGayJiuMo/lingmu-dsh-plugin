// LingMu relay usage report - Client half (open-source edition).
//
// This file is a plain-JavaScript Cordis plugin body: the exact text you feed
// to the DSH dynamic-plugin tools (cordis_define -> code.client). It renders a
// draggable floating window in the shell.overlay seat:
//   - drag the title bar to move; released within 64px of a page edge it snaps
//     to that edge, otherwise it stays where you dropped it (free float),
//   - the '-' button minimizes it into a 46px 'ling' floating ball; dragging
//     the ball docks it to the nearest horizontal edge, clicking it expands,
//   - the gear button opens an account settings view where users enter their
//     own LingMu email/password; they are saved in browser localStorage
//     (key lingmu.creds.v1) and forwarded to the Host with every report call.
//     No credentials are hardcoded anywhere.
// The overlay layer is click-through, so the widget opts back in with
// pointer-events:auto. Drag uses pointer capture on the head/ball elements;
// presses landing on a button never start a drag (capture would swallow the
// click). Drag-session state lives in module-scope vars (single root-scoped
// overlay instance); the DOM node is captured via a callback ref.
...
.lm-days-input{width:56px;height:24px;padding:0 6px;box-sizing:border-box;border-radius:999px;border:1px solid var(--dsw-alias-border-l1);background:transparent;color:var(--dsw-alias-label-primary);font-size:12px;line-height:1;text-align:center;-moz-appearance:textfield}
.lm-days-input::-webkit-inner-spin-button,.lm-days-input::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}
...
function LingMuFloat() {
  ...
  React.useEffect(function () {
    ...
    setLoading(true);
    setError(null);
    host.call('lingmu-report', { days: days, creds: creds })
      .then(function (res) {
        if (cancelled) return;
        if (res && res.ok === true) {
          setData(res);
  ...
  // open (same size as the shortcut tabs, so the row never grows);
  // Enter/blur confirms (button then reads 近X天 and highlights),
